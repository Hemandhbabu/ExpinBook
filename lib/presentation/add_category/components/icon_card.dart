...
class IconCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final icon = ref.watch(AddCategoryProvider.iconProvider);
    final symbol = ref.watch(AddCategoryProvider.symbolProvider);
    final editable = ref.watch(AddCategoryProvider.editableProvider);
    return AddEditInfoCard(
      title: 'Symbol',
      alignment: CrossAxisAlignment.center,
      onTap: editable ? () => showOptions(context, ref.read) : null,
...
void showOptions(BuildContext context, Reader read) => showCustomAction(
      ...
      TileActionData(
        ...
        valueChanged: (value) {
          if (value == 0) {
            showIconPicker(context, read(AddCategoryProvider.iconProvider))
                .then(
              (value) => value?.let(
                (value) {
                  read(AddCategoryProvider.iconProvider.notifier)
                      .update((state) => value);
                  read(AddCategoryProvider.symbolProvider.notifier)
                      .update((state) => null);
                },
              ),
            );
          } else if (value == 1) {
            showSymbolGetter(context, read(AddCategoryProvider.symbolProvider))
                .then(
              (value) => value?.let(
                (value) {
                  read(AddCategoryProvider.iconProvider.notifier)
