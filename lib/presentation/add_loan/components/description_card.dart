...
class DescriptionCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final description = ref.watch(AddLoanProvider.descriptionProvider);
    return AddEditInfoCard(
      title: 'Description',
      onTap: () => showDescriptionGetter(context, description).then(
        (value) => value?.let(
          (value) => ref
              .read(AddLoanProvider.descriptionProvider.notifier)
              .update((state) => value),
        ),
      ),
      child: CustomTile(
