...
class AmountCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final amount = ref.watch(AddLoanTransactionProvider.amountProvider);
    final editable = ref.watch(AddLoanTransactionProvider.editableProvider);
    return AddEditInfoCard(
      title: 'Amount',
      onTap: editable
          ? () => showAmountGetter(context, amount).then(
                (value) => value?.let(
                  (value) => ref
                      .read(AddLoanTransactionProvider.amountProvider.notifier)
                      .update((state) => value),
                ),
              )
          : null,
