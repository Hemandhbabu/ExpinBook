...
class ToPaymentCard extends ConsumerWidget {
  const ToPaymentCard({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ref.listen(
      ...
      (previous, next) {
        ref
            .read(AddTransactionProvider.toPaymentProvider.notifier)
            .update((state) => next == TransactionType.transfer ? 1 : null);
      },
    );
    final toPayment = ref.watch(AddTransactionProvider.toPaymentProvider);
    if (toPayment == null) return const SizedBox.shrink();
    final payment = ref.watch(_paymentProvider(toPayment));
    final editable = ref.watch(AddTransactionProvider.editableProvider);
    return AsyncWidgetWrapper(
      asyncValue: payment,
      useNullBuilder: false,
      builder: (context, payment) => Padding(
          ...
          onTap: editable
              ? () => showSinglePaymentPicker(context, payment).then(
                    (value) => value?.let(
                      (value) => ref
                          .read(
                              AddTransactionProvider.toPaymentProvider.notifier)
